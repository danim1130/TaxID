...
def run_multiple_and_get_output(images,
    ...
    for image in images:
        temp_name, img_extension = save_image(image)
        image_names.append(temp_name + os.extsep + img_extension)
    ...
    try:
        ...
        kwargs = {
            ...
            'output_filename_base': temp_list_name + '_out',
            'extension': extension,
            'extension_configs': extension_configs,
            'lang': lang,
            'config': config,
...
def image_to_string(image,
    ...
    if output_type == Output.DICT:
        return {'text': run_and_get_output(image, extension, lang, config, nice)}
    elif output_type == Output.BYTES:
        return run_and_get_output(image, extension, lang, config, nice, True)

    return run_and_get_output(image, extension, extension_configs, lang, config, nice)
